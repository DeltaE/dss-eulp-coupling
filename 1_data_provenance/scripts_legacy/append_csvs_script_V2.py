...
residential_columns = [
    ...
    'in.city', 'in.county', 'in.vacancy_status', 'in.geometry_building_type_acs',
...
list_of_state_folders = list(set([i for i in os.listdir() if 'Metadata' in i]))
allowed_states = ['MI', 'MT', 'VT', 'WA', 'MN']
allowed_subfolders = [i for i in os.listdir() if 'Metadata' in i and i.split('_')[1] in allowed_states]
BOOL_FILTER_STATES = True
# BOOL_FILTER_STATES = False
if BOOL_FILTER_STATES:
    pass
else:
    allowed_subfolders = list_of_state_folders

allowed_subfolders_commercial = [i for i in allowed_subfolders if 'Commercial' in i]
allowed_subfolders_residential = [i for i in allowed_subfolders if 'Residential' in i and '_2' not in i]
allowed_subfolders_residential_2 = [i for i in allowed_subfolders if 'Residential_2' in i]

# print('get up until here')
# sys.exit()

# Traverse through all folders and subfolders
for foldername in allowed_subfolders:  # Use '.' to refer to the current directory

    files_inside_subfolders = [i for i in os.listdir('./' + foldername) if i.endswith('.csv')]

    # print('get until here 1')
    # sys.exit()

    for filename in files_inside_subfolders:
        if 'baseline' in filename:
            file_path = os.path.join(foldername, filename)
            
            # print('get until here 2')
            # sys.exit()
            state_ID = foldername.split('_')[1]

            # Determine if the folder is for commercial or residential files
            if 'Commercial' in foldername and foldername in allowed_subfolders:
            ...
            elif 'Residential' in foldername and 'Residential_2' not in foldername and foldername in allowed_subfolders:
...
if commercial_dataframes:
    ...
    if BOOL_FILTER_STATES:
        commercial_dataframe.to_csv('commercial_data_SELECT_STATES_MI_MT_VT_WA.csv', index=False)
    else:
        commercial_dataframe.to_csv('commercial_data_SELECT_MI_MT_VT_WA.csv', index=False)
...
if residential_dataframes:
    ...
    if BOOL_FILTER_STATES:
        residential_dataframe.to_csv('residential_data_SELECT_STATES_MI_MT_VT_WA.csv', index=False)
    else:
        residential_dataframe.to_csv('residential_data_SELECT_MI_MT_VT_WA.csv', index=False)
