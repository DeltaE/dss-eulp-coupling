...
# json upgrades: https://oedi-data-lake.s3.amazonaws.com/nrel-pds-building-stock/end-use-load-profiles-for-us-building-stock/2024/comstock_amy2018_release_1/upgrades_lookup.json

# technical release: https://oedi-data-lake.s3.amazonaws.com/nrel-pds-building-stock/end-use-load-profiles-for-us-building-stock/2024/resstock_tmy3_release_2/resstock_documentation_2024_release_2.pdf

# tmy3: https://www.nrel.gov/docs/fy08osti/43156.pdf

# Comstock packages: https://nrel.github.io/ComStock.github.io/docs/upgrade_measures/upgrade_measures.html

# Modeling specifics: https://www.nrel.gov/docs/fy24osti/89128.pdf

# Important: https://www.nrel.gov/docs/fy20osti/75215.pdf
'''
...
LIST_INCLUDE_UPGRADES_RES = ['baseline', 'upgrade01', 'upgrade02', 'upgrade03',
                             'upgrade04']
LIST_INCLUDE_UPGRADES_COM = ['upgrade19', 'upgrade20']
list_upgrades_reformat = [i.replace('upgrade0', 'upgrade=') 
    for i in LIST_INCLUDE_UPGRADES_RES if i != 'baseline']
...
if BOOL_DOWNLOAD_METADATA_RESI:
    for a_state in file_names_raw_META:
        ...
        for acsv in csv_file_urls_all:
            for up_str in LIST_INCLUDE_UPGRADES_RES:
                if up_str in acsv:
...
if BOOL_DOWNLOAD_METADATA_COMM:
    for a_state in file_names_raw_META:
        ...
        file_links = \
            driver_5.find_elements(By.XPATH, "//a[contains(@href, '.csv')]")
        csv_file_urls_all_UNFILTERED = [link.get_attribute('href') for link in file_links]

        csv_file_urls_all = [i for i in csv_file_urls_all_UNFILTERED if 'basic' not in i]

        csv_file_urls = []
        for acsv in csv_file_urls_all:
            for up_str in LIST_INCLUDE_UPGRADES_COM:
                if up_str in acsv:
                    csv_file_urls.append(acsv)

        str_state = a_state.split('=')[-1].replace('/', '')
        download_folder = 'Metadata_' + str(str_state) + '_Commercial_2'
        download_any_files(csv_file_urls, download_folder)
