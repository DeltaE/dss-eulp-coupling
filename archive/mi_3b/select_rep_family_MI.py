...
desired_buildings_per_type = {
   ...
   "LargeHotel": 13, # round(0.02*len(unique_com_parquets)),
   "SecondarySchool": 0, #round(0.03*len(unique_com_parquets)),
   "LargeOffice": 21, #round(0.04*len(unique_com_parquets)),
   "MediumOffice": 34, #round(0.05*len(unique_com_parquets)),
   "SmallHotel": 64, # round(0.05*len(unique_com_parquets)),
   "PrimarySchool": 0, #round(0.05*len(unique_com_parquets)),
   "QuickServiceRestaurant": 107, #round(0.10*len(unique_com_parquets)),
   "Outpatient": 107, #round(0.05*len(unique_com_parquets)),
   "FullServiceRestaurant": 107, #round(0.05*len(unique_com_parquets)),
   "RetailStandalone": 99, # round(0.15*len(unique_com_parquets)),
   "RetailStripmall": 107, #round(0.10*len(unique_com_parquets)),
   "SmallOffice": 112, #round(0.15*len(unique_com_parquets)),
   "Warehouse": 107 #round(0.15*len(unique_com_parquets)) + 2 # makes dictionary equal to the total
}

total_buildings = sum(desired_buildings_per_type.values())
print('Total buildings: ', total_buildings, '/', len(unique_com_parquets))
...
custom_order = [
   "Hospital",
   # "SecondarySchool", THERE SEEM TO BE NO SECONDARY SCHOOLS IN THE MICHIGAN DATASET
   "LargeHotel",
   # "PrimarySchool", THERE SEEM TO BE NO PRIMARY SCHOOLS IN THE MICHIGAN DATASET
   "LargeOffice",
   "SmallHotel",
   "MediumOffice",
   "RetailStandalone",
