...
if FILTER_RESIDENTIAL:
    ...
    df_res = pd.read_csv("../residential_data_SELECT_STATES_MI_MT_VT_WA.csv")
    df_res_filtered = df_res[df_res["bldg_id"].isin(unique_bldg_ids_res)].copy()
    ...
    df_res_filtered.loc[:, "income_group"] = df_res_filtered["in.area_median_income"].apply(
...
if FILTER_COMMERCIAL:
    ...
    df_com = pd.read_csv("../commercial_data_SELECT_STATES_MI_MT_VT_WA.csv")
    df_com_filtered = df_com[df_com["bldg_id"].isin(unique_bldg_ids_com)].copy()
