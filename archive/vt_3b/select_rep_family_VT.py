...
desired_buildings_per_type = {
   "Hospital": 1, #round(0.01*len(unique_com_parquets)),
   "LargeHotel": 7, # round(0.02*len(unique_com_parquets)),
   "SecondarySchool": 3, #round(0.03*len(unique_com_parquets)),
   "LargeOffice": 7*5, #round(0.04*len(unique_com_parquets)),
   "MediumOffice": 8*5, #round(0.05*len(unique_com_parquets)),
   "SmallHotel": 17*5, # round(0.05*len(unique_com_parquets)),
   "PrimarySchool": 3, #round(0.05*len(unique_com_parquets)),
   "QuickServiceRestaurant": 12*5, #round(0.10*len(unique_com_parquets)),
   "Outpatient": 10*5, #round(0.05*len(unique_com_parquets)),
   "FullServiceRestaurant": 19*5, #round(0.05*len(unique_com_parquets)),
   "RetailStandalone": 12*5, # round(0.15*len(unique_com_parquets)),
   "RetailStripmall": 45*3, #round(0.10*len(unique_com_parquets)),
   "SmallOffice": 0, #round(0.15*len(unique_com_parquets)),
   "Warehouse": 65*2 #round(0.15*len(unique_com_parquets)) + 2 # makes dictionary equal to the total
}
...
counts_df = commercial_df['in.comstock_building_type'].value_counts().reset_index()
counts_df.columns = ['building_type', 'count']
# Sort smallest to largest
counts_df = counts_df.sort_values('count', ascending=True).reset_index(drop=True)
# Add total row at the end
total_row = pd.DataFrame({'building_type': ['TOTAL'], 'count': [counts_df['count'].sum()]})
...
# ------------------------------------------------------------
# TROUBLESHOOT MISSING COMMERCIAL PARQUETS
# Assign any remaining parquets even if it exceeds per-type quotas
# ------------------------------------------------------------
BOOL_ALLOW_MISSING_PARQUETS_COM = True
if missing_parquets_com and BOOL_ALLOW_MISSING_PARQUETS_COM:
    print("\n=== Final Attempt to Assign Missing Commercial Parquets ===")

    # Reverse lookup: Map Parquet -> Possible bldg_ids from `com_exploded`
    parquet_to_bldg_ids_com = (
        com_exploded[com_exploded["Unique_Source_Files"].isin(missing_parquets_com)]
        .groupby("Unique_Source_Files")["bldg_id"]
        .apply(set)
        .to_dict()
    )

    for parquet_file in list(missing_parquets_com):  # list() to avoid set mutation during iteration
        possible_bldg_ids = parquet_to_bldg_ids_com.get(parquet_file, set())

        # Find a matching building in `commercial_df`
        match_candidates = commercial_df[commercial_df["bldg_id"].isin(possible_bldg_ids)]
        ...
        if not match_candidates.empty:
            ...
            selected_rows.append(chosen_row_data)

            # Remove from missing set
            missing_parquets_com.discard(parquet_file)

            print(f"✅ Assigned {parquet_file} -> bldg_id {chosen_row_data['bldg_id']} ({chosen_row_data['in.comstock_building_type']})")
        else:
            print(f"❌ No candidate found for {parquet_file}")

    # Rebuild final_commercial with the new rows
    final_commercial = pd.DataFrame(selected_rows)
    
    # Recalculate used/missing
    used_parquets_com = set(final_commercial["Chosen_Parquet"].unique())
    missing_parquets_com = set(unique_com_parquets) - used_parquets_com

    print(f"\nAfter troubleshooting: used_parquets={len(used_parquets_com)}, missing_parquets={len(missing_parquets_com)}")
...
ordered_types_res_scenario = [
    # "Mobile Home",  # DO NOT SEEM TO APPEAR IN THE BASE DATASET
    "2 Unit",
    "3 or 4 Unit",
    "Single-Family Attached",
    "Single-Family Detached",
    "5 to 9 Unit",
    # "10 to 19 Unit",  # DO NOT SEEM TO APPEAR IN THE BASE DATASET
    # "20 to 49 Unit",  # DO NOT SEEM TO APPEAR IN THE BASE DATASET
    # "50 or more Unit",  # DO NOT SEEM TO APPEAR IN THE BASE DATASET
]
'''
ordered_types_res_scenario = [
    "Mobile Home",
...
res_desired_buildings_per_type = {
    "Single-Family Detached": round(0.71*len(unique_res_parquets)), # round(0.55*len(unique_res_parquets)),
    "Single-Family Attached": round(0.25*len(unique_res_parquets)),
    "50 or more Unit": 0, #round(0.05*len(unique_res_parquets)),
    "20 to 49 Unit": 0, #round(0.03*len(unique_res_parquets)),
    "10 to 19 Unit": 0, # round(0.03*len(unique_res_parquets)),
    "5 to 9 Unit": 0, # round(0.03*len(unique_res_parquets)),
